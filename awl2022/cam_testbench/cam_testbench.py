...
import time
import picamera
#Please Read Documentation for this library in detail
#https://picamera.readthedocs.io/en/latest/
import numpy as np
import cv2
import os #allow for directory specification
...
# below x and y res is for when taking 64 samples
xRes = 1600 
yRes = 912
...
intersect_span = 8 #Taking 12 samples on the left and 12 samples on the right for intersection detection
...
edge_threshold = 3 # must have 2 continous road samples to be considered road 
...
# set up path for save
img_path1 = r'/home/pi/Capstone_2022/cam_testbench/calibration_pictures/masks'
img_path2 = r'/home/pi/Capstone_2022/cam_testbench/calibration_pictures/processed'
...
#         hsv_range = [avgH1 - 30, avgS1 - 40,  avgV1 - 55, avgH1 + 30, avgS1 + 40, avgV1 + 55]
...
def navigate(hsv_range):
    for n in range (0, 30):
        with picamera.PiCamera() as camera:
            ...
            #changing directory to save  masks
            os.chdir(img_path1)
            cv2.imwrite('Mask' + imgNum + '.png', mask1)
            ...
            if aligned == True:
                ...
                for i in range (0,intersect_span): 
                    ...
                    if i == left_intersect_end - 1:
                        ...
                        if (left_sum > sample_threshold * intersect_span): # if intersection is detected on the left side
                    ...
                    if i == num_sample - 1:
                        # now check if left intersection exists 
                        if (right_sum > sample_threshold * intersect_span): # if road is detected in the outer eight regions on the right side
            ...
            road_center = (int) (halfX + (xAngle / xPixAngle))

            center1 = cv2.line(img1, (road_center,y_top), (road_center,y_bot), (0,255,0), 2)
            cropLine = cv2.line(center1, (0,halfY), (xRes,halfY), (255,0,255), 2)
            regions = cv2.rectangle(cropLine,(calib_col,calib_row),(calib_col+calibrator_dim-1,calib_row+calibrator_dim-1), (255,0,0), 2)
            for i in range (0,num_sample):
                    
                # if within road boundaries,draw filled green squares
                if (i >= left_edge_index and i <= right_edge_index):
                    cv2.rectangle(regions,(left_col+i*left2right_col,yCropTop+sample_dim+height_btwn_samples),(left_col+i * left2right_col+sample_dim,yCropBot), (0,255,0), -1)
                else:
                    cv2.rectangle(regions,(left_col+i*left2right_col,yCropTop+sample_dim+height_btwn_samples),(left_col+i * left2right_col+sample_dim,yCropBot), (0,255,0), 2)
                
            for i in range (0,intersect_span): 
                    cv2.rectangle(regions,(left_col+i*left2right_col,yCropTop),(left_col+i * left2right_col+sample_dim,yCropTop+sample_dim), (0,0,255), 2)
                    cv2.rectangle(regions,(xRes-left_col-i*left2right_col,yCropTop),(xRes-left_col-i*left2right_col-sample_dim,yCropTop+sample_dim), (0,0,255), 2)
                    #Color code both left and right ends if intersection exists. Blue means no intersection on that side
                    if (left_intersection == True): 
                        cv2.rectangle(regions,(left_col+i*left2right_col,yCropTop),(left_col+i * left2right_col+sample_dim,yCropTop+sample_dim), (0,255,255), -1)
                    if (right_intersection == True): 
                        cv2.rectangle(regions,(xRes-left_col-i*left2right_col,yCropTop),(xRes-left_col-i*left2right_col-sample_dim,yCropTop+sample_dim), (0,255,255), -1)
                        
            final1 = cv2.line(regions, (halfX,y_top), (halfX,y_bot), (0,0,255), 2)

            #changing directory to save processed images
            os.chdir(img_path2)

            cv2.imwrite('processed' + imgNum + '.png', final1)
            print("********************************************************")

            print("Current Iteration: " + str(n+1) + " out of 30")
            print("Low End HSV")
            print([hsv_range[0], hsv_range[1], hsv_range[2]])
            print("High End HSV")
            print([hsv_range[3], hsv_range[4], hsv_range[5]])
            print("Single Frame Process Time: " + str(diff_time))
            print("Sample Array")
            print(sample_sum)
            print("Angle: " + str(xAngle))
            print("Left Edge Found: " + str(left_edge_found))
            print("Right Edge Found: " + str(right_edge_found))
            print("Left Edge Index: " + str(left_edge_index))
            print("Right Edge Index: " + str(right_edge_index)) 
            print("Left Intersection: " + str(left_intersection))
            print("Right Intersection: " + str(right_intersection))
